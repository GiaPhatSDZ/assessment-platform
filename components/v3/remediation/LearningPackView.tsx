...
import { BookOpen, ArrowRight, ShieldCheck, Clock, ShieldAlert } from "lucide-react";
...
export function LearningPackView({
  ...
  // If learning content is not published / draft, fail closed to CONTENT_NOT_AVAILABLE view
  if (pack.contentStatus === "CONTENT_NOT_AVAILABLE") {
    return (
      <div data-testid="learning-pack-not-available" className="mx-auto max-w-4xl px-4 py-8 space-y-8 animate-fadeIn">
        <div className="rounded-2xl border border-amber-200 bg-white p-6 sm:p-8 shadow-xs dark:border-slate-800 dark:bg-slate-900 space-y-6">
          <div className="inline-flex items-center gap-1.5 rounded-full border border-amber-200 bg-amber-50 px-3 py-0.5 text-xs font-semibold text-amber-800 dark:bg-amber-950/40 dark:text-amber-300">
            <Clock className="h-3.5 w-3.5 text-amber-600" />
            <span>CONTENT_NOT_AVAILABLE · HỌC LIỆU ĐANG THẨM ĐỊNH</span>
          </div>
          ...
          <div className="rounded-xl border border-amber-200/70 bg-amber-50/40 p-4 sm:p-5 space-y-3 dark:border-amber-900/40 dark:bg-amber-950/20">
            <div className="flex items-center gap-2 text-xs font-bold text-amber-900 dark:text-amber-200">
              <ShieldAlert className="h-4 w-4 text-amber-700" />
              <span>Học liệu hướng dẫn đang được thẩm định sư phạm</span>
            </div>
            <p className="text-xs sm:text-sm text-slate-700 dark:text-slate-300 leading-relaxed">
              Theo quy chuẩn an toàn R2.2, toàn bộ nội dung hướng dẫn chi tiết, ví dụ giải mẫu và bài tập luyện tập cho mắt xích <strong>{pack.focusNodeLabel}</strong> phải được phê duyệt chính thức bởi chuyên gia sư phạm con người trước khi phát hành cho học sinh.
            </p>
            <p className="text-xs text-slate-500 dark:text-slate-400">
              Hệ thống từ chối hiển thị tài liệu Tier-D hoặc nội dung tự sinh khi chưa qua cổng kiểm duyệt xuất bản.
            </p>
          </div>
          ...
          <div className="pt-4 flex flex-col sm:flex-row items-center justify-between gap-3 border-t border-warm-100 dark:border-slate-800">
            {onBackToReport && (
              <button
                type="button"
            ...
            >
              <span>Tiến hành kiểm tra lại (Re-test)</span>
  ...
  // Published learning pack view (when content is fully reviewed and published)
  return (
    <div className="mx-auto max-w-4xl px-4 py-8 space-y-8">
      ...
      {/* Worked Examples (Only when published) */}
      {pack.workedExamplePlan.length > 0 && (
